...
if [ -z "${FILE_SERVER_BASE_URL}" ]; then
	if [ ! -z "${FILE_SERVER_BASE_IP}" ]; then
		if [ ! -z "${FILE_SERVER_BASE_PORT}" ]; then
			export FILE_SERVER_BASE_URL="https://${FILE_SERVER_BASE_IP}:${FILE_SERVER_BASE_PORT}"
		else
			export FILE_SERVER_BASE_URL="https://${FILE_SERVER_BASE_IP}:${FILE_SERVER_LISTEN_PORT}"
		fi
	fi
fi

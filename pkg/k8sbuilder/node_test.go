...
func TestNodeBuildContainer(t *testing.T) {
	user := int64(7051)
	gt := NewGomegaWithT(t)
	gt.Expect(nodeBuildContainer("hyperledger/fabric-nodeenv:2.0")).To(Equal(v1.Container{
...
func TestNodeRunContainer(t *testing.T) {
	seccontext := getSecContext()
	gt := NewGomegaWithT(t)
	rmd := &runMetadata{PeerAddress: "peer-address:9999"}
	gt.Expect(nodeRunContainer(rmd, "hyperledger/fabric-nodeenv:2.0")).To(Equal(v1.Container{
		Name:            "run-node-chaincode",
		Image:           "hyperledger/fabric-nodeenv:2.0",
		ImagePullPolicy: v1.PullAlways,
		Command:         []string{"/bin/sh"},
		Args:            []string{"-c", fmt.Sprintf(nodeStartScript, "peer-address:9999")},
		Env:             rmd.toRunEnv(),
		VolumeMounts: []v1.VolumeMount{
